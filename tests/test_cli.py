...
def test_list_voices() -> None:
    ...
    try:
        ...
        # Using "gen-tts" assumes it's in the path (which it is if installed via
        # uv tool or pip install -e .)
...
def test_no_input_error() -> None:
    ...
    try:
        ...
        assert result.returncode != 0
        assert (
            "No input provided." in result.stderr or
            "the following arguments are required" in result.stderr
        )
